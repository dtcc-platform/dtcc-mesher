...
static char *dtcc_mesher_with_suffix(const char *base, const char *suffix)
{
    char *path;
...
static int dtcc_mesher_parse_double_arg(const char *text, double *out_value)
{
    char *end = NULL;
...
static int dtcc_mesher_parse_size_arg(const char *text, size_t *out_value)
{
    char *end = NULL;
...
static void dtcc_mesher_print_usage(FILE *stream)
{
    fprintf(
        stream,
...
static int dtcc_mesher_print_error(const char *context, const char *path, dtcc_mesher_status status, const dtcc_mesher_error *error)
{
    const char *detail = dtcc_mesher_status_string(status);
...
int main(int argc, char **argv)
{
    ...
    while (argi < argc && argv[argi][0] == '-') {
        if (strcmp(argv[argi], "-h") == 0 || strcmp(argv[argi], "--help") == 0) {
            dtcc_mesher_print_usage(stdout);
            return EXIT_SUCCESS;
        }
        ...
        if (strcmp(argv[argi], "-v") == 0 || strcmp(argv[argi], "--verbose") == 0) {
            ...
            options.acute_protection_mode = DTCC_MESHER_ACUTE_PROTECTION_NONE;
        } else if (strcmp(argv[argi], "--simple-acute-protection") == 0) {
            options.enable_acute_protection = 1;
            options.acute_protection_mode = DTCC_MESHER_ACUTE_PROTECTION_SIMPLE;
            ...
            argi += 1;
            if (argi >= argc || !dtcc_mesher_parse_double_arg(argv[argi], &options.protect_angle_deg)) {
                dtcc_mesher_print_usage(stderr);
                return EXIT_FAILURE;
            }
        } else if (strcmp(argv[argi], "--max-refine-steps") == 0) {
            argi += 1;
            if (argi >= argc || !dtcc_mesher_parse_size_arg(argv[argi], &options.max_refinement_steps)) {
                dtcc_mesher_print_usage(stderr);
                return EXIT_FAILURE;
            }
        } else if (strcmp(argv[argi], "--max-protection-levels") == 0) {
            argi += 1;
            if (argi >= argc || !dtcc_mesher_parse_size_arg(argv[argi], &options.max_protection_levels)) {
                dtcc_mesher_print_usage(stderr);
                return EXIT_FAILURE;
            }
        } else {
    ...
    if (!dtcc_mesher_has_suffix(input_path, ".pts") && !dtcc_mesher_has_suffix(input_path, ".pslg")) {
        fprintf(stderr, "unsupported input type for %s\n", input_path);
        return EXIT_FAILURE;
    }

    status = dtcc_mesher_read_domain_file(input_path, &domain, &error);
    if (status != DTCC_MESHER_STATUS_OK) {
        return dtcc_mesher_print_error("failed to read", input_path, status, &error);
    }

    status = dtcc_mesher_generate(&domain, &options, &mesh, &error);
    if (status != DTCC_MESHER_STATUS_OK) {
        dtcc_mesher_domain_free(&domain);
        return dtcc_mesher_print_error("failed to mesh", input_path, status, &error);
    }

    tri_path = dtcc_mesher_with_suffix(out_base, ".tri");
    svg_path = dtcc_mesher_with_suffix(out_base, ".svg");
    metrics_path = dtcc_mesher_with_suffix(out_base, ".metrics.csv");
    summary_path = dtcc_mesher_with_suffix(out_base, ".summary.txt");
    if (tri_path == NULL || svg_path == NULL || metrics_path == NULL || summary_path == NULL) {
        fprintf(stderr, "failed to allocate output paths\n");
        goto cleanup;
    }

    status = dtcc_mesher_write_triangles(&mesh, tri_path, &error);
    if (status != DTCC_MESHER_STATUS_OK) {
        exit_code = dtcc_mesher_print_error("failed to write", tri_path, status, &error);
        goto cleanup;
    }

    status = dtcc_mesher_write_svg(&mesh, svg_path, &error);
    if (status != DTCC_MESHER_STATUS_OK) {
        exit_code = dtcc_mesher_print_error("failed to write", svg_path, status, &error);
        goto cleanup;
    }

    status = dtcc_mesher_write_quality_csv(&mesh, metrics_path, &error);
    if (status != DTCC_MESHER_STATUS_OK) {
        exit_code = dtcc_mesher_print_error("failed to write", metrics_path, status, &error);
        goto cleanup;
    }

    status = dtcc_mesher_write_quality_summary(&mesh, summary_path, &error);
    if (status != DTCC_MESHER_STATUS_OK) {
        exit_code = dtcc_mesher_print_error("failed to write", summary_path, status, &error);
        goto cleanup;
    }
